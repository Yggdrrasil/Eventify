...
// Relación entre Categoría y Evento
Categoria.hasMany(Evento, { foreignKey: 'categoriaId', onDelete: 'SET NULL' });
...
async function syncDatabase() {
    try {
      ...
      await Categoria.sync({ alter: true }); // Primero Categorias
...
app.post('/inscripciones', authMiddleware, async (req, res) => {
  try {
    ...
    if (creador) {
      await Notificacion.create({
        tipo: 'Inscripción',
        contenido: `El usuario ${req.user.nombre} se ha inscrito en tu evento "${evento.nombre}"`,
  ...
  app.delete('/inscripciones/:id', authMiddleware, async (req, res) => {
    ...
    // Crear una categoría (POST /categorias)
  ...
  app.get('/categorias/:id', authMiddleware, async (req, res) => {
    try {
  ...
  // Actualizar una categoría por ID (PUT /categorias/:id)
